...
class VmConfig(betterproto.Message):
    ...
    wasmtime -> wamr. Available Wasm runtime types are registered as
    ...
    _extension_envoy.wasm.runtime.wasmtime: **envoy.wasm.runtime.wasmtime**:
    `Wasmtime <https://wasmtime.dev/>`_-based WebAssembly runtime. This runtime
    is not enabled in the official build. [#extension-category:
    envoy.wasm.runtime]
    """
    ...
    calls. Note that these functions are generally called implicitly by your
    language's standard library. Therefore, you do not need to call them
    directly. You can access environment variables in the same way you would on
    native platforms. Warning: Envoy rejects the configuration if there's
    conflict of key space.
    """
