...
class JwtProvider(betterproto.Message):
    ...
    seconds: 300 [#next-free-field: 22]
    """
    ...
    match the ``iss`` field in JWT, otherwise the JWT ``iss`` field is not
    checked. Note: ``JwtRequirement`` :ref:`allow_missing <envoy_v3_api_field_e
    xtensions.filters.http.jwt_authn.v3.JwtRequirement.allow_missing>` and
    ...
    differently than other ``JwtRequirements``. Hence the usage of this field
    is different as follows if ``allow_missing`` or ``allow_missing_or_failed``
    is used: * If a JWT has ``iss`` field, it needs to be specified by this
    field in one of ``JwtProviders``. * If a JWT doesn't have ``iss`` field,
    one of ``JwtProviders`` should fill this field empty. * Multiple
    ``JwtProviders`` should not have same value in this field. Example:
    ...
    subjects: "_____type_matcher_v3__.StringMatcher" = betterproto.message_field(19)
    """
    Restrict the `subjects
    <https://tools.ietf.org/html/rfc7519#section-4.1.2>`_ that the JwtProvider
    can assert. For instance, this could implement JWT-SVID `subject
    restrictions <https://github.com/spiffe/spiffe/blob/main/standards/JWT-
    SVID.md#31-subject>`_. If not specified, will not check subjects in the
    token. Example: .. code-block:: yaml     subjects:       prefix:
    spiffe://spiffe.example.com/
    """

    require_expiration: bool = betterproto.bool_field(20)
    """
    Requires that the credential contains an `expiration
    <https://tools.ietf.org/html/rfc7519#section-4.1.4>`_. For instance, this
    could implement JWT-SVID `expiration restrictions
    <https://github.com/spiffe/spiffe/blob/main/standards/JWT-
    SVID.md#33-expiration-time>`_. Unlike ``max_lifetime``, this only requires
    that expiration is present, where ``max_lifetime`` also checks the value.
    Example: .. code-block:: yaml     require_expiration: true
    """

    max_lifetime: timedelta = betterproto.message_field(21)
    """
    Restrict the maximum remaining lifetime of a credential from the
    JwtProvider. Credential lifetime is the difference between the current time
    and the expiration of the credential. For instance, the following example
    will reject credentials that have a lifetime longer than 24 hours. If not
    set, expiration checking still occurs, but there is no limit on credential
    lifetime. If set, takes precedence over ``require_expiration``. Example: ..
    code-block:: yaml     max_lifetime:       seconds: 86400
    """
    ...
    caveat: only works for from_header/from_params & has no effect for JWTs
    extracted through from_cookies.
    """
    ...
    ``fields`` and the value is the ``protobuf::Struct`` converted from JWT
    JSON payload. For example, if payload_in_metadata is ``my_payload``: ..
    ...
    normalize_payload_in_metadata: "JwtProviderNormalizePayload" = (
    ...
    (``protobuf::Struct``) in ``envoy.filters.http.jwt_authn`` ``namespace``
    with the value of this field as the key. For example, if
    ``header_in_metadata`` is ``my_header``: .. code-block:: yaml
    envoy.filters.http.jwt_authn:     my_header:       alg: JWT       kid:
    EF71iSaosbC5C4tC6Syq1Gm647M       alg: PS256 When the metadata has
    ``envoy.filters.http.jwt_authn`` entry already (for example if
    :ref:`payload_in_metadata <envoy_v3_api_field_extensions.filters.http.jwt_a
    uthn.v3.JwtProvider.payload_in_metadata>` is not empty), it will be
    inserted as a new entry in the same ``namespace`` as shown below: .. code-
    ...
    failed_status_in_metadata: str = betterproto.string_field(16)
    """
    If non empty, the failure status ``::google::jwt_verify::Status`` for a non
    verified JWT will be written to StreamInfo DynamicMetadata in the format
    as: ``namespace`` is the jwt_authn filter name as
    ``envoy.filters.http.jwt_authn`` The value is the ``protobuf::Struct``. The
    values of this field will be ``code`` and ``message`` and they will contain
    the JWT authentication failure status code and a message describing the
    failure. For example, if failed_status_in_metadata is
    ``my_auth_failure_status``: .. code-block:: yaml
    envoy.filters.http.jwt_authn:     my_auth_failure_status:       code: 3
    message: Jwt expired
    """
    ...
    claim_to_headers: List["JwtClaimToHeader"] = betterproto.message_field(15)
    """
    Add JWT claim to HTTP Header Specify the claim name you want to copy in
    which HTTP header. For examples, following config: The claim must be of
    type; string, int, double, bool. Array type claims are not supported ..
    literalinclude:: /_configs/repo/jwt_authn.yaml    :language: yaml
    :lines: 44-48    :linenos:    :lineno-start: 44    :caption:
    :download:`jwt_authn.yaml </_configs/repo/jwt_authn.yaml>` This header is
    only reserved for jwt claim; any other value will be overwritten.
    """

    clear_route_cache: bool = betterproto.bool_field(17)
    """
    Clears route cache in order to allow JWT token to correctly affect routing
    decisions. Filter clears all cached routes when: 1. The field is set to
    ``true``. 2. At least one ``claim_to_headers`` header is added to the
    request OR    if ``payload_in_metadata`` is set.
    """


@dataclass(eq=False, repr=False)
class JwtProviderNormalizePayload(betterproto.Message):
    """
    Alters the payload representation in the request dynamic metadata to
    facilitate its use in matching.
    """

    space_delimited_claims: List[str] = betterproto.string_field(1)
    """
    Each claim in this list will be interpreted as a space-delimited string and
    converted to a list of strings based on the delimited values. Example: a
    token with a claim ``scope: "email profile"`` is translated to dynamic
    metadata  ``scope: ["email", "profile"]`` if this field is set value
    ``["scope"]``. This special handling of ``scope`` is recommended by
    `RFC8693 <https://datatracker.ietf.org/doc/html/rfc8693#name-scope-scopes-
    claim>`_.
    """
...
class JwksAsyncFetch(betterproto.Message):
    ...
    failed_refetch_duration: timedelta = betterproto.message_field(2)
    """
    The duration to refetch after a failed fetch. If not specified, default is
    1 second.
    """
...
class FilterStateRule(betterproto.Message):
    ...
    This FilterState should use ``Router::StringAccessor`` object to set a
    string value. Other HTTP filters can use it to specify Jwt requirements
...
class JwtAuthentication(betterproto.Message):
    ...
    provider1              - provider_name: provider2 [#next-free-field: 7]
    """
    ...
    n.v3.PerRouteConfig.requirement_name>` in ``PerRouteConfig`` uses this map
    to specify a JwtRequirement.
    """

    strip_failure_response: bool = betterproto.bool_field(6)
    """
    A request failing the verification process will receive a 401 downstream
    with the failure response details in the body along with WWWAuthenticate
    header value set with "invalid token". If this value is set to true, the
    response details will be stripped and only a 401 response code will be
...
class PerRouteConfig(betterproto.Message):
    ...
    MUST be specified at the :ref:`requirement_map <envoy_v3_api_field_extensio
    ns.filters.http.jwt_authn.v3.JwtAuthentication.requirement_map>` in
    ``JwtAuthentication``. If no, the requests using this route will be
    rejected with 403.
    """


@dataclass(eq=False, repr=False)
class JwtClaimToHeader(betterproto.Message):
    """This message specifies a combination of header name and claim name."""

    header_name: str = betterproto.string_field(1)
    """
    The HTTP header name to copy the claim to. The header name will be
    sanitized and replaced.
    """

    claim_name: str = betterproto.string_field(2)
    """
    The field name for the JWT Claim : it can be a nested claim of type (eg.
    "claim.nested.key", "sub") String separated with "." in case of nested
    claims. The nested claim name must use dot "." to separate the JSON name
    path.
    """
