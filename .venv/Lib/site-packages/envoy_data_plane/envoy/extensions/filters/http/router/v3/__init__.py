...
from ....network.http_connection_manager import (
    v3 as ___network_http_connection_manager_v3__,
)
...
class Router(betterproto.Message):
    ...
    false. .. attention::   This field is deprecated by the
    :ref:`spawn_upstream_span <envoy_v3_api_field_extensions.filters.network.ht
    tp_connection_manager.v3.HttpConnectionManager.Tracing.spawn_upstream_span>
    `.   Please use that ``spawn_upstream_span`` field to control the span
    creation.
    """

    upstream_log: List["_____config_accesslog_v3__.AccessLog"] = (
    ...
    upstream_log_options: "RouterUpstreamAccessLogOptions" = betterproto.message_field(
        9
    )
    ...
    This only affects the :ref:`router filter generated x-envoy- headers
    ...
    upstream_http_filters: List[
        "___network_http_connection_manager_v3__.HttpFilter"
    ...
    for all requests that pass through the router. They will also be applied to
    shadowed requests. Upstream HTTP filters cannot change route or cluster.
...
@dataclass(eq=False, repr=False)
class RouterUpstreamAccessLogOptions(betterproto.Message):
    flush_upstream_log_on_upstream_stream: bool = betterproto.bool_field(1)
    """
    If set to true, an upstream access log will be recorded when an upstream
    stream is associated to an http request. Note: Each HTTP request received
    for an already established connection will result in an upstream access log
    record. This includes, for example, consecutive HTTP requests over the same
    connection or a request that is retried. In case a retry is applied, an
    upstream access log will be recorded for each retry.
    """

    upstream_log_flush_interval: timedelta = betterproto.message_field(2)
    """
    The interval to flush the upstream access logs. By default, the router will
    flush an upstream access log on stream close, when the HTTP request is
    complete. If this field is set, the router will flush access logs
    periodically at the specified interval. This is especially useful in the
    case of long-lived requests, such as CONNECT and Websockets. The interval
